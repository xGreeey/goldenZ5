...
(function () {
    ...
    const STORAGE_THEME = 'hr-admin-theme';
    const STORAGE_SIDEBAR = 'hr-admin-sidebar-collapsed';
    ...
    /* -------------------------------------------------------------------------
       ...
       ------------------------------------------------------------------------- */
    const STORAGE_SECTION_TEAM = 'hr-admin-section-team-collapsed';
    const sectionMenuBtn = document.getElementById('sectionMenuBtn');
    const sectionTeam = document.getElementById('hr-sidebar-section-team');
    const sectionTeamList = document.getElementById('hr-sidebar-section-team-list');
    ...
    /* -------------------------------------------------------------------------
       ...
       ------------------------------------------------------------------------- */
    const STORAGE_STATUS = 'hr-admin-status';
    ...
    var STATUS_CONFIG = {
        active: { label: 'Active', dotClass: 'hr-sidebar-status-dot-active' },
        busy: { label: 'Busy', dotClass: 'hr-sidebar-status-dot-busy' },
        dnd: { label: 'Do not disturb', dotClass: 'hr-sidebar-status-dot-dnd' },
        brb: { label: 'Be right back', dotClass: 'hr-sidebar-status-dot-brb' },
        away: { label: 'Appear away', dotClass: 'hr-sidebar-status-dot-away' },
        offline: { label: 'Appear offline', dotClass: 'hr-sidebar-status-dot-offline' }
    };
    ...
    function setStatus(statusKey) {
        ...
        if (statusDot) {
            statusDot.className = 'hr-sidebar-status-dot ' + config.dotClass;
        }
        if (statusLabel) {
            statusLabel.textContent = config.label;
        }
        var options = statusPopup && statusPopup.querySelectorAll('.hr-sidebar-status-option[data-status]');
        if (options) {
    ...
    if (statusTrigger && statusPopup) {
        var statusWrap = statusTrigger.closest('.hr-sidebar-status-wrap');
        ...
        statusPopup.querySelectorAll('.hr-sidebar-status-option').forEach(function (btn) {
    ...
    if (userMenuTrigger && userMenuPopup) {
        const footer = userMenuTrigger.closest('.hr-sidebar-footer');
        const POPUP_GAP = 8;
    ...
    (function initTasksTabs() {
        var wrap = document.querySelector('.hr-tasks-tabs-wrap[data-tablist="tasks"]');
        if (!wrap) return;

        var tablist = wrap.querySelector('[role="tablist"]');
        var tabs = wrap.querySelectorAll('[role="tab"]');
        var panelsContainer = document.querySelector('.hr-tasks-panels');

...
?>
<div class="hr-page hr-page-documents">
    <header class="hr-page-header">
        <nav class="hr-breadcrumb" aria-label="Breadcrumb">
            <ol class="hr-breadcrumb-list">
                ...
                <li class="hr-breadcrumb-item hr-breadcrumb-current" aria-current="page">Documents</li>
            </ol>
        </nav>
        <div class="hr-page-header-actions">
            <a href="<?php echo htmlspecialchars($base_url); ?>?page=employees" class="hr-btn hr-btn-ghost">Employees</a>
        </div>
    </header>

    <section class="hr-section">
        <h2 class="hr-section-title">Upload document</h2>
        <?php if ($upload_success): ?>
            <div class="hr-alert hr-alert-success">
                <div class="hr-alert-icon" aria-hidden="true"><i class="fas fa-check"></i></div>
                <div class="hr-alert-body">
                    <p class="hr-alert-message">Document uploaded successfully.</p>
                </div>
            </div>
        <?php endif; ?>
        <?php if ($delete_success): ?>
            <div class="hr-alert hr-alert-success">
                <div class="hr-alert-icon" aria-hidden="true"><i class="fas fa-check"></i></div>
                <div class="hr-alert-body">
                    <p class="hr-alert-message">Document deleted.</p>
                </div>
            </div>
        <?php endif; ?>
        <?php if ($upload_error !== ''): ?>
            <div class="hr-alert hr-alert-error">
                ...
                <div class="hr-alert-body">
                    <p class="hr-alert-message"><?php echo htmlspecialchars($upload_error); ?></p>
                </div>
            </div>
        <?php endif; ?>
        <form method="post" enctype="multipart/form-data" action="" class="hr-form">
            <input type="hidden" name="document_upload" value="1">
            <div class="hr-form-grid">
                <div class="hr-form-group">
                    <label for="employee_id">Employee <span class="hr-required">*</span></label>
                    <select id="employee_id" name="employee_id" class="hr-select" required>
                ...
                <div class="hr-form-group">
                    ...
                    <select id="document_type" name="document_type" class="hr-select">
                ...
                <div class="hr-form-group">
                    ...
                    <input type="text" id="category" name="category" class="hr-input" placeholder="e.g. contract, ID, certification" value="<?php echo htmlspecialchars($_POST['category'] ?? ''); ?>">
                </div>
                <div class="hr-form-group">
                    <label for="document_file">File <span class="hr-required">*</span></label>
                    <input type="file" id="document_file" name="document_file" class="hr-input" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" required>
                    <span class="hr-text-muted">PDF, DOC, DOCX, JPG, PNG. Max 10MB.</span>
                </div>
            </div>
            <div class="hr-form-actions">
                <button type="submit" class="hr-btn hr-btn-primary">Upload</button>
            </div>
        </form>
    </section>

    <section class="hr-section">
        <h2 class="hr-section-title">Documents</h2>
        <form method="get" action="<?php echo htmlspecialchars($base_url); ?>" class="hr-filters">
            ...
            <select name="type" class="hr-select">
            ...
            <button type="submit" class="hr-btn hr-btn-secondary">Filter</button>
        </form>

        <?php if (empty($documents)): ?>
            <p class="hr-text-muted">No documents found. Upload above or run <code>database/schema/phase1_hr.sql</code> if the Documents table is missing.</p>
        <?php else: ?>
                    ...
                    <tbody>
                        <?php foreach ($documents as $doc): ?>
                            <tr>
                                ...
                                <td><span class="hr-badge hr-badge-neutral"><?php echo htmlspecialchars($doc['document_type']); ?></span></td>
                                ...
                                <td>
                                    <a href="<?php echo htmlspecialchars($base_url); ?>/document-download.php?id=<?php echo (int)$doc['id']; ?>" class="hr-btn hr-btn-ghost hr-btn-sm">Download</a>
                                    <form method="post" action="" class="hr-inline-form" onsubmit="return confirm('Delete this document?');">
                                        ...
                                        <button type="submit" class="hr-btn hr-btn-ghost hr-btn-sm hr-btn-danger">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
